...
final class RootActor(config: Config) extends Actor with ActorLogging {
  ...
  private val playerEngine = context.spawn(PlayerEngine(), PlayerEngine.Name)

  private val api = {
    import config.api._
    context.spawn(Api(address, port, playerEngine, askTimeout), Api.Name)
  }

  context.watch(api)
  context.watch(playerEngine)
